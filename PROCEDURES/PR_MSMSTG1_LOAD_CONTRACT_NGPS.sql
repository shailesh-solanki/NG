--------------------------------------------------------
--  DDL for Procedure PR_MSMSTG1_LOAD_CONTRACT_NGPS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSMSTG1_LOAD_CONTRACT_NGPS" 
/**************************************************************************************
*
* Program Name           :PR_MSMSTG1_LOAD_CONTRACT_NGPS
* Author                 :IBM
...
flag number := 0;
 CURSOR c_ci_sa IS
SELECT distinct usi.sa_id,' ' AS prop_dcl_rsn_cd,' ' AS prop_sa_id,'ANC' AS cis_division,
 CASE
...
css.asb_comp_code AS acct_id,
 TO_DATE(substr(fn_convert_gmt_bst1(contract_end),1,11),'DD-MON-RRRR') AS end_dt FROM trn_con_d1_us_ngps css, connect_us_sa_id_ngps usi  
 WHERE   css.contract_seq =usi.contract_seq and css.asb_unit_code=usi.asb_unit_code AND 
 contract_status IN ('C','T')
 AND css.load_seq_nbr=pi_load_seq_nbr;

 cursor c_us_sa_id is
select us_id,contract_seq
    from connect_us_sa_id_ngps a where load_seq_nbr = pi_load_seq_nbr;

cursor c_sa_id is
select us_id,a.sa_id
    from connect_us_sa_id_ngps a, msm_stg1.ci_sa b where a.sa_id = b.sa_id and b.load_seq_nbr = pi_load_seq_nbr;

cursor c_con_num is
select dui.us_id,sup.contract_number from msm_stg1.d1_us_identifier dui, srs_supplier_stg sup ,connect_us_sa_id_ngps cui
where dui.id_value = sup.contract_seq
and dui.id_value=cui.contract_seq
and dui.us_id=cui.us_id 
and sup.asb_unit_code=cui.asb_unit_code
and dui.us_id_type_flg = 'CONS'
and sup.contract_number is not null
and dui.load_seq_nbr = pi_load_seq_nbr;

cursor c_comp_code is
select dui.us_id,acs.asb_comp_code from msm_stg1.d1_us_identifier dui, asb_contract_service_ngps acs , connect_us_sa_id_ngps usi
where dui.id_value = acs.contract_seq and dui.id_value=usi.contract_seq and dui.us_id=usi.us_id
and dui.us_id_type_flg = 'CONS'
...
as contract_end
    FROM connect_us_sa_id_ngps csi , asb_contract_service_ngps acs, srs_supplier_stg sup, msm_stg1.ci_sa cs
    where acs.CONTRACT_SEQ=csi.CONTRACT_SEQ  
    and sup.contract_seq = acs.contract_seq 
    and cs.sa_id = csi.sa_id 
    and csi.asb_unit_code=sup.asb_unit_code 
    and acs.LOAD_SEQ_NBr=PI_LOAD_SEQ_NBR;


CURSOR c_con_id IS 
select distinct contract_seq from srs_contract_factor_stg group by contract_seq having count(1) > 0;
...
from srs_despatch_stg des, connect_us_sa_id_ngps cus 
where cus.contract_seq = des.contract_seq
and cus.load_seq_nbr = pi_load_seq_nbr;
...
from srs_supplier_stg sup, connect_us_sa_id_ngps cus ,srs_despatch_stg des
where cus.contract_seq = sup.contract_seq
and cus.contract_seq = des.contract_seq
and cus.asb_unit_code= sup.asb_unit_code
and cus.load_seq_nbr = pi_load_seq_nbr;
...
from asb_contract_service_ngps acs, connect_us_sa_id_ngps cus, srs_despatch_stg des, SRS_SUPPLIER_STG sup
...
from srs_seasonal_stg sea, connect_us_sa_id_ngps cus, srs_despatch_stg des
...
and acr.contract_seq = p_contract_seq) as available,
(select max(acr.CONTRACT_RATE_1) from asb_contract_rate_stg acr where pay_code = 'SRBU' and contract_seq = acs.contract_seq 
and acr.contract_seq = p_contract_seq) as utilization,'' as premium,
sup.OPTN_CAPACITY,SYSDATE
from asb_contract_service_ngps acs,  connect_us_sa_id_ngps cus,srs_supplier_stg sup
...
select distinct contract_seq from asb_contract_rate_stg ;
BEGIN
/*
...
delete from connect_us_sa_id_ngps where sa_id not in (select sa_id from msm_stg1.ci_sa where load_seq_nbr = pi_load_seq_nbr);
...
dbms_output.put_line('D1_US_IDENTIFIER start'||sysdate);
 ...
 /*case when trn_con_d1_us_ngps.CONTRACT_STATUS in ('C','T') 
 then 'ACTIVE' 
 when trn_con_d1_us_ngps.CONTRACT_STATUS = 'D' 
 then 'INACTIVE'  end*/
 /*'ACTIVE' as BO_STATUS_CD,' ',
 CASE
 WHEN  trn_con_d1_us_ngps.SERVICE_CODE='SRBM' and TENDER_NUMBER='99.99'THEN 'STOR_BM_PREQUAL'
 WHEN trn_con_d1_us_ngps.SERVICE_CODE='SRNM' and trn_con_d1_us_ngps.TENDER_NUMBER='99.99' THEN 'STOR_NBM_PREQUAL'
 WHEN trn_con_d1_us_ngps.SERVICE_CODE='SRBM' and trn_con_d1_us_ngps.COMM_FLEX='C' AND trn_con_d1_us_ngps.TENDER_NUMBER<>'99.99' THEN 'STOR_BM_CONTRACT_COMMITTED'
 WHEN trn_con_d1_us_ngps.SERVICE_CODE='SRNM' and trn_con_d1_us_ngps.COMM_FLEX='C' AND trn_con_d1_us_ngps.TENDER_NUMBER<>'99.99' THEN 'STOR_NBM_CONTRACT_COMMITTED'
 WHEN trn_con_d1_us_ngps.SERVICE_CODE='SRNM' and trn_con_d1_us_ngps.COMM_FLEX='F' AND trn_con_d1_us_ngps.TENDER_NUMBER<>'99.99' THEN 'STOR_NBM_FLEXIBLE'
 END
 ,'OUC2M','D1NR',fn_convert_gmt_bst1(trn_con_d1_us_ngps.CONTRACT_START),
fn_convert_gmt_bst1(trn_con_d1_us_ngps.CONTRACT_END) ,
 fn_convert_gmt_bst1(SYSDATE),fn_convert_gmt_bst1(SYSDATE),99,'D1AC','UK',
 '<tenderNumber>'||trn_con_d1_us_ngps.tender_number||'</tenderNumber><flexiComIndicator>'||trn_con_d1_us_ngps.COMM_FLEX||'</flexiComIndicator><maxOccPerWeek>'||trn_con_d1_us_ngps.MAX_OCCS_WEEK
 ||'</maxOccPerWeek><maxHrsPerWeek>'||trn_con_d1_us_ngps.MAX_HOURS_WEEK||'</maxHrsPerWeek><maxOccPerYear>'||trn_con_d1_us_ngps.MAX_OCCS_CONT||'</maxOccPerYear><maxHrsPerYear>'||
 trn_con_d1_us_ngps.MAX_HOURS_CONT||'</maxHrsPerYear><maxUtilPeriod>'||trn_con_d1_us_ngps.MAX_UTILISATION||'</maxUtilPeriod><nearestNode>'||trn_con_d1_us_ngps.NEAREST_NODE||'</nearestNode><location>'||
 trn_con_d1_us_ngps.LOCATION||'</location><northOrSouth>'||trn_con_d1_us_ngps.NORTH_SOUTH||'</northOrSouth><includeInContractDataExtract>'||trn_con_d1_us_ngps.INCLUDE_CONTRACT||
 '</includeInContractDataExtract><instructionMechanism>'||trn_con_d1_us_ngps.INSR_MECH||'</instructionMechanism>',
 '','','','','',sysdate 
  FROM trn_con_d1_us_ngps , connect_us_sa_id_ngps usi  WHERE   trn_con_d1_us_ngps.contract_seq =usi.contract_seq and trn_con_d1_us_ngps.asb_unit_code=usi.asb_unit_code AND  contract_status IN ('C','T') AND trn_con_d1_us_ngps.load_seq_nbr=PI_LOAD_SEQ_NBR;
...
dbms_output.put_line('D1_US end'||sysdate);
   ...
   /* for rec in c_d1_us_char_sea
    ...
    select pi_load_seq_nbr,dui.us_id,cui.asb_unit_code,fn_convert_gmt_bst1(acs.contract_start),--decode(acs.contract_status,'D',fn_convert_gmt_bst1(acs.contract_end),''),
    fn_convert_gmt_bst1(acs.contract_end),'D1AD',100,99,null,null,null,sysdate 
    from MSM_STg1.D1_US_IDENTIFIER dui, asb_contract_service_ngps acs , connect_us_sa_id_ngps cui
    where acs.contract_seq=cui.contract_seq 
    and dui.id_value = acs.contract_seq 
    and dui.id_value=cui.contract_seq and dui.us_id=cui.us_id 
    and dui.us_id_type_flg = 'CONS'
    and dui.load_seq_nbr = pi_load_seq_nbr;
 ...
 FROM MSM_STG1.D1_US_SP DUS, connect_us_sa_id_ngps cus ,srs_supplier_stg sup
 where dus.us_id = cus.us_id 
 and cus.contract_seq = sup.contract_seq and cus.asb_unit_code=sup.asb_unit_code  and DUS.LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR;
 ...
 ----------------------------------------------------------------
    ...
    LOOP
       ...
       SELECT pi_load_seq_nbr,cus.us_id,'CM_STDBY_TOLERANCE',fn_convert_gmt_bst1(scf.EFFECTIVE),
        ...
        ,99,SYSDATE 
        FROM srs_contract_factor_stg scf, connect_us_sa_id_ngps cus where cus.contract_seq = scf.contract_seq and scf.contract_seq = rec.contract_seq
       and cus.load_seq_nbr = pi_load_seq_nbr  ;
...
SELECT pi_load_seq_nbr,SQ_CONTRACT_D1_US_QTY_ID.NEXTVAL,E.US_ID,
  ...
  FROM asb_contract_service_ngps A,srs_supplier_stg B,asb_contract_rate_stg C,connect_us_sa_id_ngps E
...
INSERT INTO  MSM_STG1.D1_US_QTY_LOG(LOAD_SEQ_NBR,D1_US_QTY_ID,SEQNO,LOG_DTTM,LOG_ENTRY_TYPE_FLG,DESCRLONG,BO_STATUS_CD,BO_STATUS_REASON_CD,
  ...
  '','','MIGD','99',SYSDATE FROM msm_stg1.D1_US_QTY where load_seq_nbr = pi_load_seq_nbr;
...
/*      insert into MSM_STG1.D1_US_LOG
             ...
             from connect_us_sa_id_ngps cus,asb_contract_service_ngps acs where cus.contract_seq = acs.contract_seq and acs.contract_status not in ('D') and acs.LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR;
     ...
     /*insert into MSM_STG1.D1_US_LOG
             ...
             from connect_us_sa_id_ngps cus,asb_contract_service_ngps acs where cus.contract_seq = acs.contract_seq and acs.contract_status in ('D') and acs.LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR;*/
  ...
  /*  insert into MSM_STG1.D1_US_LOG_PARM
    select distinct pi_load_seq_nbr,US_ID,1,2,'Active',' ',99,sysdate from connect_us_sa_id_ngps cus,asb_contract_service_ngps acs 
    where cus.contract_seq = acs.contract_seq and acs.contract_status not in ('D') and acs.LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR;
*/
   /* insert into MSM_STG1.D1_US_LOG_PARM
    select distinct pi_load_seq_nbr,US_ID,2,3,'Inactive',' ',99,sysdate from connect_us_sa_id_ngps cus,asb_contract_service_ngps acs 
    ...
    EXCEPTION
      WHEN OTHERS then
        ...
        ASB_STG.PR_PROCESS_LOG('PR_MSMSTG1_LOAD_CONTRACT_NGPS',pi_LOAD_SEQ_NBR,'FAILURE',v_ERROR);
        DBMS_OUTPUT.PUT_LINE('Error --> '||SQLERRM);

END PR_MSMSTG1_LOAD_CONTRACT_NGPS;

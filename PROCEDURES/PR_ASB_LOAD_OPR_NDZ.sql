--------------------------------------------------------
--  DDL for Procedure PR_ASB_LOAD_OPR_NDZ
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_ASB_LOAD_OPR_NDZ" 
/**************************************************************************************
*
* Program Name           :PR_ASB_OPR_LOAD_NDZ
...
BEGIN
  ...
  MERGE INTO PN_NOTICE_STG A
   USING (
--   select * FROM PN_NOTICE WHERE (ASB_UNIT_CODE,EFFECTIVE,ITEM_CODE)IN (select asb_unit_code, max(effective) AS EFFECTIVE,ITEM_CODE from PN_NOTICE WHERE ITEM_CODE='NDZ'
-- group by asb_unit_code,ITEM_CODE having max(effective)< PI_EFFECTIVE)
-- UNION
 SELECT ASB_UNIT_CODE, ITEM_CODE, EFFECTIVE, PERIOD, FILE_DATE from PN_NOTICE where ITEM_CODE = 'NDZ' AND TRUNC(EFFECTIVE)>=PI_EFFECTIVE AND TRUNC(EFFECTIVE) < TRUNC(PI_END_DATE) + 1) q
 ON (q.ASB_UNIT_CODE = A.ASB_UNIT_CODE AND q.ITEM_CODE = A.ITEM_CODE AND q.EFFECTIVE = A.EFFECTIVE)
    ...
    WHEN NOT MATCHED THEN
     ...
     ASB_STG.PROC_PROCESS_LOG('PR_ASB_OPR_NDZ_LOAD',pi_load_seq_nbr,'SUCCESS', 'Operational(NDZ) data migrated successfully from ASB legacy system to ASB_STG schema','NDZ');

EXCEPTION
   WHEN OTHERS then
        ASB_STG.PROC_PROCESS_LOG('PR_ASB_OPR_NDZ_LOAD',pi_load_seq_nbr,'FAILURE', SUBSTR(sqlerrm,1,3000),'NDZ');

END PR_ASB_LOAD_OPR_NDZ;

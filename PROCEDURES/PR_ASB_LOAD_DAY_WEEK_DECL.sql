--------------------------------------------------------
--  DDL for Procedure PR_ASB_LOAD_DAY_WEEK_DECL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_ASB_LOAD_DAY_WEEK_DECL" (PI_LOAD_SEQ_NBR NUMBER,PI_CUT_OFF_DATE IN DATE,PI_END_DATE IN DATE) 
/**************************************************************************************
*
* Program Name           :PR_ASB_LOAD_DAY_WEEK_DECL
...
* Called Program         :PR_ASB_LOAD_DAY_WEEK_DECL_MAIN
*
...
BEGIN
 ...
 MERGE INTO SRD_DECLARATION_STG_DW S2
    USING (select sd.ASB_UNIT_CODE, sd.CONTRACT_NUMBER, sd.SERVICE_DATE, sd.SERVICE_PERIOD, 
            ...
            from srd_declaration sd , srs_supplier_STG sup , asb_contract_service_STG cs 
            where 
    ...
    WHERE  
        --(nvl(S2.AVAIL_WEEK,'checknull') <> nvl(S1.AVAIL_WEEK,'checknull')) OR 
                       --(nvl(s2.AVAIL_LEVEL,'checknull') <> nvl(s1.AVAIL_LEVEL,'checknull')) OR 
                       (nvl(s2.FILE_TYPE,'checknull') <> nvl(s1.FILE_TYPE,'checknull')) OR
                       --(nvl(s2.REVISED_WEEK,'checknull') <> nvl(s1.REVISED_WEEK,'checknull')) OR
                       --(nvl(s2.REVISED_LEVEL,'checknull') <> nvl(s1.REVISED_LEVEL,'checknull')) OR
                       ...
                       (nvl(s2.DAY_CODE,'checknull') <> nvl(s1.DAY_CODE,'checknull')) --OR
                       --(nvl(s2.contract_seq,'checknull') <> nvl(s1.contract_seq,'checknull'))
    ...
    PROC_PROCESS_LOG('PR_ASB_LOAD_DAY_WEEK_DECL',pi_load_seq_nbr,'SUCCESS','All the new records pushed to SRD_DECLARATION_STG_DW table sucessfully!!!','DAYWEEKDECLARATION');

EXCEPTION WHEN NO_DATA_FOUND THEN
         v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,200);
         DBMS_OUTPUT.PUT_LINE('ERROR : ' || v_ERROR);
        PROC_PROCESS_LOG('PR_ASB_LOAD_DAY_WEEK_DECL',pi_load_seq_nbr,'SUCCESS','No insert or update done in SRD_DECLARATION_STG_DW table','DAYWEEKDECLARATION');
    WHEN OTHERS THEN   
         v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,200);
         DBMS_OUTPUT.PUT_LINE('ERROR : ' || v_ERROR);
         PROC_PROCESS_LOG('PR_ASB_LOAD_DAY_WEEK_DECL',PI_LOAD_SEQ_NBR,'FAILURE', 'Failed while migrating data from legacy to ASB_STG schema','DAYWEEKDECLARATION');


END PR_ASB_LOAD_DAY_WEEK_DECL;

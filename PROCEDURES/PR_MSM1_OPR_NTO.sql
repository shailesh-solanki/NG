--------------------------------------------------------
--  DDL for Procedure PR_MSM1_OPR_NTO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSM1_OPR_NTO" (PI_LOAD_SEQ_NBR IN NUMBER)
AS 
/**************************************************************************************
*
* Program Name           :PR_MSM1_OPR_NTO
...
* Called Program         :PR_ASB_LOAD_OPR_NTO_MAIN
*
*
* Input files            :None
* Output files           :None
...
CURSOR cur_NTO_CNT2 IS 
select distinct ASB_UNIT_CODE from PN_NOTICE_STG where ITEM_CODE = 'NTO' and load_seq_nbr = PI_LOAD_SEQ_NBR group by ASB_UNIT_CODE having count(1) > 0;

BEGIN

    -- Inserting records in D1_SP_QTY table.
    FOR rec in cur_NTO_CNT2
    LOOP
       ...
       -- Inserting records in D1_SP_QTY (MSM_STG1)
        ...
        SELECT PI_LOAD_SEQ_NBR,SQ_OPS_D1_SP_QTY_ID.NEXTVAL,ASB_UNIT_CODE,'NTO',EFFECTIVE,
        --(select min(effective) from PN_NOTICE_STG where ASB_UNIT_CODE = PN1.ASB_UNIT_CODE and ITEM_CODE = PN1.ITEM_CODE and load_seq_nbr = PN1.load_seq_nbr and effective > PN1.effective )
        LEAD(EFFECTIVE) OVER(PARTITION BY ASB_UNIT_CODE ORDER BY ASB_UNIT_CODE,EFFECTIVE)AS END_DTTM
        ...
        and ITEM_CODE = 'NTO' and load_seq_nbr = PI_LOAD_SEQ_NBR  ;
       END LOOP; 
        ...
        ,SYSDATE 
        FROM MSM_STG1.D1_SP_QTY  where D1_SP_QTY_TYPE_CD = 'NTO' and load_seq_nbr = PI_LOAD_SEQ_NBR  ;
    ...
    WHERE d1_sp_qty_type_cd = 'NTO' AND load_seq_nbr = pi_load_seq_nbr;
     ...
     EXCEPTION
      ...
      ROLLBACK;
       v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,200);
        PROC_PROCESS_LOG('PR_MSM1_OPR_NTO',pi_LOAD_SEQ_NBR,'FAILURE',v_ERROR,'NTO');

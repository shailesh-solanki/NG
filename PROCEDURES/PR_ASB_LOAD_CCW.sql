--------------------------------------------------------
--  DDL for Procedure PR_ASB_LOAD_CCW
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_ASB_LOAD_CCW" (PI_LOAD_SEQ_NBR NUMBER,PI_CUT_OFF_DATE IN DATE,PI_END_DATE IN DATE) 
/**************************************************************************************
*
...
* Description            :This is a PL/SQL procedure. This procedure cops data from legacy table(SRD_DECLARATION) and loads 
                          into ASB_STG table(SRD_DECLARATION_STG)
*                         
...
BEGIN
 ...
 --Loads data into SRD_DECLARATION_STG Table in ASB_STG schema
    MERGE INTO SRD_DECLARATION_STG S2
...
--                       (nvl(s2.FILE_DATE,'checknull') <> nvl(s1.FILE_DATE,'checknull')) OR
...
--                       (nvl(s2.contract_seq,'checknull') <> nvl(s1.contract_seq,'checknull'))
    ...
    ,SYSDATE);
     ...
     PROC_PROCESS_LOG('PR_ASB_LOAD_CCW',pi_load_seq_nbr,'SUCCESS','All the new records pushed to SRD_DECLARATION_STG table sucessfully.','CCW');
     
EXCEPTION 
    WHEN NO_DATA_FOUND THEN
         v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,400);
         DBMS_OUTPUT.PUT_LINE('ERROR : ' || v_ERROR);
        PROC_PROCESS_LOG('PR_ASB_LOAD_CCW',pi_load_seq_nbr,'SUCCESS','No insert or update done in  SRD_DECLARATION_STG table','CCW');
    WHEN OTHERS THEN   
         v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,400);
         DBMS_OUTPUT.PUT_LINE('ERROR : ' || v_ERROR);

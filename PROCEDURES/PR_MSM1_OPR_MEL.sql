--------------------------------------------------------
--  DDL for Procedure PR_MSM1_OPR_MEL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSM1_OPR_MEL" (PI_LOAD_SEQ_NBR IN NUMBER,p_MEASR_COMP_TYPE_CD IN VARCHAR2,p_RANK IN VARCHAR2)
/**************************************************************************************
*
* Program Name           :PR_MSM1_OPR_MEL
...
*  26-08-2021     Roshan Khandare   Changes for MEL Data 
**************************************************************************************/
AS
 -- V_MC_COMP_ID NUMBER;
 -- V_SETT_DATE DATE;
...
CURSOR cur_MEL (v_asb_unit_code VARCHAR2)
IS 
    select 
    ...
    where 
        ...
        and dmc.measr_comp_type_cd = p_MEASR_COMP_TYPE_CD  --'MELSTARTMIN'
        and pas.item_code = 'MEL'
        and pas.rank = p_RANK 
...
BEGIN
    ...
    select distinct asb_unit_code bulk collect into lv_vc_unit_code from pn_availability_stg where LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR and item_code = 'MEL' and rank = p_RANK ;

    for i in 1..lv_vc_unit_code.count
    loop
      FOR REC IN cur_MEL (lv_vc_unit_code(i))
      LOOP    
        IF (REC.PREV_MSRMT_DTTM IS NULL) THEN
            ...
            AND ITEM_CODE = 'MEL' AND effective < REC.effective;
        END IF ;

        INSERT INTO MSM_STG1.D1_MSRMT_MEL

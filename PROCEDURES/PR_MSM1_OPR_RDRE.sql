--------------------------------------------------------
--  DDL for Procedure PR_MSM1_OPR_RDRE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSM1_OPR_RDRE" (PI_LOAD_SEQ_NBR IN NUMBER)
AS 
/**************************************************************************************
*
* Program Name           :PR_MSM1_OPR_RDRE
...
* Description            :This is a PL/SQL procedure. This procedure calls other procedures populate the records 
                          into D1_SP_QTY(MSM_STG1) from PN_RANGE_STG table.
* Calling Program        :None
* Called Program         :PR_ASB_LOAD_OPR_MAIN.ksh
*
*
* Input files            :None
* Output files           :None
...
CURSOR cur_RDRE_CNT2 IS 
select distinct ASB_UNIT_CODE from PN_RANGE_STG where ITEM_CODE = 'RDRE' and load_seq_nbr = PI_LOAD_SEQ_NBR group by ASB_UNIT_CODE having count(1) > 0;

BEGIN

    -- Inserting records in D1_SP_QTY table.
    FOR rec in cur_RDRE_CNT2
    LOOP
       ...
       SELECT PI_LOAD_SEQ_NBR,SQ_OPS_D1_SP_QTY_ID.NEXTVAL,ASB_UNIT_CODE,'RDRE',EFFECTIVE,
        ...
        FROM PN_RANGE_STG PN1 where ASB_UNIT_CODE = rec.ASB_UNIT_CODE
        and ITEM_CODE = 'RDRE' and load_seq_nbr = PI_LOAD_SEQ_NBR  ;
       END LOOP; 
       ...
       SELECT PI_LOAD_SEQ_NBR,D1_SP_QTY_ID,1,sysdate,'F1CR',NULL,NULL,NULL,11002,12152,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'MIGD',99
        ,SYSDATE 
        FROM MSM_STG1.D1_SP_QTY  where D1_SP_QTY_TYPE_CD = 'RDRE' and load_seq_nbr = PI_LOAD_SEQ_NBR  ;
...
INSERT INTO msm_stg1.d1_sp_qty_log (load_seq_nbr,d1_sp_qty_id,seqno,log_dttm,log_entry_type_flg,descrlong,bo_status_cd,bo_status_reason_cd,message_cat_nbr,message_nbr,char_type_cd,
    ...
    WHERE d1_sp_qty_type_cd = 'RDRE' AND load_seq_nbr = pi_load_seq_nbr;
     ...
     EXCEPTION
      ...
      ROLLBACK;
       v_ERROR:=SQLCODE||' '||SUBSTR(sqlerrm,1,200);
        PR_PROCESS_LOG('PR_MSM1_OPR_RDRE',pi_LOAD_SEQ_NBR,'FAILURE',v_ERROR);
        DBMS_OUTPUT.PUT_LINE('Error --> '||SQLERRM);

END PR_MSM1_OPR_RDRE;

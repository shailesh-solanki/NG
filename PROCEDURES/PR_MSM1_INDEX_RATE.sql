--------------------------------------------------------
--  DDL for Procedure PR_MSM1_INDEX_RATE
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSM1_INDEX_RATE" 
(
...
CASE
            ...
            END FACTOR_CD
from asb_contract_service_stg a,srs_supplier_stg b,connect_us_sa_id e,
...
*/
--value for within contract
select * from (
select distinct c.load_seq_nbr,a.contract_seq,a.contract_start  as effective,c.index_rate,
case 
    when a.contract_end<=nvl(c.end_effective,a.contract_end) then a.contract_end
    else c.end_effective 
    END END_DATE,
CASE
                WHEN c.SERVICE_CODE='SRBM' AND c.PAY_CODE='SRBA' THEN 'SRP_STOR_AVAIL_INDEXRATE'
                WHEN c.SERVICE_CODE='SRNM' AND c.PAY_CODE='SRNA' THEN 'SRP_STOR_AVAIL_INDEXRATE'
...
lead(effective) over (partition by asb_unit_code,service_code,pay_code order by effective,asb_unit_code,service_code,pay_code desc) 
as end_effective from asb_index_rate_stg) c,asb_index_rate_stg d
where a.contract_start >= c.effective and contract_start < nvl(c.end_effective,sysdate)-- and a.contract_end <= c.end_effective
and c.effective = d.effective
and c.asb_unit_code = d.asb_unit_code
and c.pay_code = d.pay_code
and c.asb_unit_code = b.asb_unit_code
--and c.pay_code = 'SRNU'
--and c.asb_unit_code = 'GFLA-1'
and b.contract_seq = a.contract_seq
and b.contract_seq = e.contract_seq
and c.INDEX_RATE<>1 
and c.PAY_CODE IN ('SRBA','SRNA','SRBU','SRNU','SRBP','SRNP')
...
--order by effective desc
union
--value for within after contract change
select * from (
select distinct c.load_seq_nbr,a.contract_seq,c.effective as
effective,c.index_rate,a.contract_end,
CASE
                WHEN c.SERVICE_CODE='SRBM' AND c.PAY_CODE='SRBA' THEN 'SRP_STOR_AVAIL_INDEXRATE'
                WHEN c.SERVICE_CODE='SRNM' AND c.PAY_CODE='SRNA' THEN 'SRP_STOR_AVAIL_INDEXRATE'
                WHEN c.SERVICE_CODE='SRBM' AND c.PAY_CODE='SRBU' THEN 'SRP_STOR_UTIL_INDEXRATE'
...
lead(effective) over (partition by asb_unit_code,service_code,pay_code order by effective,asb_unit_code,service_code,pay_code desc) 
as end_effective from asb_index_rate_stg) c,asb_index_rate_stg d
where c.effective > a.contract_start and c.effective < a.contract_end 
and c.effective = d.effective
and c.asb_unit_code = d.asb_unit_code
and c.pay_code = d.pay_code
and c.asb_unit_code = b.asb_unit_code
...
and                  LOAD_SEQ_NBR = PI_LOAD_SEQ_NBR;
     /*SELECT
         B.CONTRACT_SEQ,B.EFFECTIVE, B.INDEX_RATE,
             B.FACTOR_CD,
         fn_convert_gmt_bst1(LEAD(EFFECTIVE) OVER ( PARTITION BY CONTRACT_SEQ,ASB_PAYEE_CODE, ASB_UNIT_CODE, PAY_CODE, SERVICE_CODE ORDER BY EFFECTIVE, ASB_PAYEE_CODE, ASB_UNIT_CODE, PAY_CODE, SERVICE_CODE )) END_DATE
     FROM
           (
...
--   D1_US_FACTOR_OVRD  Table Migration
   FOR AVAIL IN IDX_RATE
    ...
    select us_id into v_us_id from connect_us_sa_id where contract_seq=AVAIL.contract_seq;
     ...
     VALUES(pi_load_seq_nbr, v_US_ID, AVAIL.FACTOR_CD, fn_convert_gmt_bst1(AVAIL.EFFECTIVE), fn_convert_gmt_bst1(AVAIL.END_DATE),AVAIL.INDEX_RATE, '99', SYSDATE);

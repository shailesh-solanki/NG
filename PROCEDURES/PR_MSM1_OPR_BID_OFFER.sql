--------------------------------------------------------
--  DDL for Procedure PR_MSM1_OPR_BID_OFFER
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ASB_STG"."PR_MSM1_OPR_BID_OFFER" (PI_LOAD_SEQ_NBR IN NUMBER, pi_bo_pair IN number, pi_sp_qty_type_cd IN varchar2)
/**************************************************************************************
*
...
CURSOR CUR_BID_OFFER IS
          SELECT ASB_UNIT_CODE AS D1_SP_ID,pi_sp_qty_type_cd as D1_SP_QTY_TYPE_CD,start_dttm,end_dttm ,NVL(BID_PRICE,0) AS D1_QUANTITY1,NVL(OFFER_PRICE,0) AS D1_QUANTITY2,
          NVL(BO_LEVEL,0) AS D1_QUANTITY3,NVL(BID_VOLUME,0) AS D1_QUANTITY4,NVL(OFFER_VOLUME,0) AS D1_QUANTITY5
   FROM SP_BID_OFFER_STG
   WHERE LOAD_SEQ_NBR=PI_LOAD_SEQ_NBR
   and bo_pair = pi_bo_pair;
